$(document).ready(function() {
	...
	$('title').html("biddenly: " + address);

	$('body').loadJSON(homeJSON);

	$('#left_img_container img').attr("src", "images/"+homeJSON.mainimg);
	$('#right_img_container img').attr("src", "images/"+homeJSON.secondaryimg);

	if (homeJSON.financial_info.hoa == 0) {		// hide the HOA row if it's 0
		$('#hoa').parent().hide();
	}

	$('#neighborhood_transitscore').html(homeJSON.area_info.transitscore);
	$('#neighborhood_walkscore').html(homeJSON.area_info.walkscore);

	// create the dots for the image scroller
	for (var i=1; i<$('.img_container').length; i++) {
		$('#dots').append('<li></li>');
	}

	var roomcount = 0;
	var obj = homeJSON.tour;
	for (var i=0; i<obj.rooms.length; i++) {
		var roomnumDisp;
		var dims;
		if (obj.rooms[i].x && obj.rooms[i].y) {
			roomcount++;
			roomnum = roomcount;
			dims = obj.rooms[i].x + "x" + obj.rooms[i].y;
		} else {
			roomnum = '';
			dims = '---';
		}
		$('#floor'+obj.rooms[i].level).append('<tr><td class="key">'+ roomnum + 
			'</td><td class="room"><a href="#room' + roomnum + '" class="room' + roomnum + 
			'_link">'+ obj.rooms[i].name + '</a></td><td class="dims"><span id="room' + roomnum + '_dims">' + dims +'</span></td></tr>');
	}

	// set the static Google map
	var mapURL = "url(http://maps.googleapis.com/maps/api/staticmap?zoom=15&size=800x200&scale=2&sensor=false&markers=color:red|" + address.split(' ').join('+') +")";
	$('#map_container').css("background-image", mapURL);
	...
	$('#footer-top').click(function (event) {
		prev_scrollPos = window.pageYOffset;
		$('html, body').animate({
		    scrollTop: 0
			},500);
		event.preventDefault();
...
function initialize() {
	...
	$.each(homeJSON.schools_section, function(i,obj) {
		var schooladdress = obj.address;
	    	...
	    	if (status == google.maps.GeocoderStatus.OK) {
				...
				directionsService.route(request, function(result, status) {
					if (status == google.maps.DirectionsStatus.OK) {
						...
						// console.log(total);
						// $('#schools .distance').eq(addressindex).html(total + " mi");
						// addressindex++;
					}
				});
